<div id="profileLogin" class="bg-black p-[60px] border-[3px] border-[#B8860B] rounded-[30px] my-[80px] max-w-[1200px] mx-auto shadow-[0_0_40px_rgba(184,134,11,0.3)] relative">

    <h2 class="text-center text-[32px] font-bold mb-[50px] text-white">Profile Login</h2>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-[40px] max-w-[900px] mx-auto">

        <div onclick="openModal('Nashwan Bima Andika', 'peserta01@gmail.com', 'blue')" 
             class="cursor-pointer group bg-gradient-to-br from-gray-600 to-gray-800 rounded-[25px] p-[45px] border-2 border-[#B8860B] text-center shadow-[0_0_30px_rgba(184,134,11,0.2)] hover:-translate-y-2 hover:shadow-[0_0_50px_rgba(184,134,11,0.4)] transition-all">
            ...
            <div class="inline-block bg-gradient-to-br from-blue-600 to-blue-900 text-white px-10 py-3 rounded-[25px] font-semibold text-[14px] uppercase tracking-wide group-hover:shadow-blue-500/50 group-hover:scale-105 transition shadow-lg">WEBSITE</div>
        </div>

        <div onclick="openModal('Vanezza Brilliance', 'peserta02@gmail.com', 'red')" 
             class="cursor-pointer group bg-gradient-to-br from-gray-600 to-gray-800 rounded-[25px] p-[45px] border-2 border-[#B8860B] text-center shadow-[0_0_30px_rgba(184,134,11,0.2)] hover:-translate-y-2 hover:shadow-[0_0_50px_rgba(184,134,11,0.4)] transition-all">
            ...
            <div class="inline-block bg-gradient-to-br from-blue-600 to-blue-900 text-white px-10 py-3 rounded-[25px] font-semibold text-[14px] uppercase tracking-wide group-hover:shadow-blue-500/50 group-hover:scale-105 transition shadow-lg">MEDIA SOSIAL</div>
        </div>
    ...
    <div id="loginModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center">
        ...
        <div class="relative bg-black border-[3px] border-[#B8860B] w-[450px] rounded-[30px] p-8 text-center shadow-[0_0_50px_rgba(184,134,11,0.6)] animate-fade-in-up">
            <div id="modalAvatarBg" class="w-[80px] h-[80px] rounded-full flex items-center justify-center mx-auto mb-4 text-[40px] shadow-[0_0_20px_rgba(255,255,255,0.2)]">👤</div>
            <h3 class="text-[#B8860B] text-[24px] font-bold mb-1">Login</h3>
            <p class="text-white text-base mb-6">Login sebagai <span id="modalNameText" class="font-bold">User</span></p>

            <div class="text-left mb-6">
                <label class="block text-gray-400 text-sm font-semibold mb-2 ml-1">Password</label>
                <input type="password" placeholder="Masukkan password..." class="w-full px-5 py-3 rounded-xl bg-[#222] text-white border border-gray-600 focus:border-[#B8860B] focus:outline-none transition-colors">
                <div class="text-right mt-2">
                    <button onclick="switchModal('loginModal', 'otpModal')" class="text-gray-400 text-xs hover:text-[#B8860B] underline transition">Lupa Password?</button>
                </div>
            </div>

            <div class="flex gap-4">
                <button onclick="closeAllModals()" class="flex-1 py-3 rounded-xl bg-[#444] text-white font-semibold hover:bg-[#555] transition">Batal</button>
                <button class="flex-1 py-3 rounded-xl bg-[#B8860B] text-white font-semibold hover:bg-[#cf980c] hover:shadow-[0_0_15px_#B8860B] transition">Login</button>
            </div>
        </div>
    </div>
    ...
    <div id="otpModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center">
        ...
        <div class="relative bg-black border-[3px] border-[#B8860B] w-[450px] rounded-[30px] p-8 text-left shadow-[0_0_50px_rgba(184,134,11,0.6)] animate-fade-in-up">
            ...
            <div class="mb-8">
                <label class="block text-white text-sm mb-2">Kode OTP</label>
...
<script>
    ...
    function openModal(name, email, colorTheme) {
        // Pastikan reset ke kondisi awal: Login muncul, yang lain hidden
        document.getElementById('loginModal').classList.remove('hidden');
        document.getElementById('otpModal').classList.add('hidden');
        document.getElementById('resetModal').classList.add('hidden');

        // Update data nama/warna
        const nameText = document.getElementById('modalNameText');
        const avatarBg = document.getElementById('modalAvatarBg');
        
        nameText.innerText = name;
        avatarBg.className = "w-[80px] h-[80px] rounded-full flex items-center justify-center mx-auto mb-4 text-[40px] shadow-[0_0_20px_rgba(255,255,255,0.2)]";
        
        if (colorTheme === 'red') {
            avatarBg.classList.add('bg-gradient-to-br', 'from-red-600', 'to-red-500');
        } else {
            avatarBg.classList.add('bg-gradient-to-br', 'from-blue-600', 'to-blue-800');
        }
    }
